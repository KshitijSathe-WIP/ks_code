...
class BackfillSQLGenerator:
    ...
    def __init__(self, target_field_id: str, graph: LineageGraph):
    ...
    def _build_mapping_cte(self, mapping_name: str, edges: list, cosmos_doc: dict | None, index: int,
        ...
        for e in edges:
            ...
            expr = self._translate_expression(e, cosmos_doc)
            alias = e["to_field"]
        ...
        where_clauses = self._extract_filters(cosmos_doc, edges)
        if where_clauses:
            body_lines.append("WHERE " + "\n  AND ".join(where_clauses))

        return {"name": cte_name, "body": "\n".join(body_lines)}

    def _translate_expression(self, edge: dict, cosmos_doc: dict | None) -> str:
    ...
    def _extract_filters(self, cosmos_doc: dict | None, edges: list) -> list:
...
def generate_backfill_sql(field_id: str, dao: LineageDataAccess = None, output_file: str = None) -> str:
    ...
    try:
        ...
        generator = BackfillSQLGenerator(field_id, graph)
